...
<div class="container mx-auto px-6">
    ...
    <div class="rounded-2xl bg-white dark:bg-gray-800 shadow-md mx-auto mt-8 p-6">
        ...
        <div class="mt-8 w-full rounded-lg border border-gray-300 dark:border-gray-600 p-4 bg-white dark:bg-gray-900 chart-container">
                ...
                <div class="max-w-3xl">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex items-center gap-2" id="scoreIconContainer">
                            ...
                            <span class="text-orange-500 font-semibold" id="scoreText">External Factor Score</span>
                        </div>

                        <!-- Subtitle -->
                        <p class="text-sm text-gray-500">
                            Weak External position. Strategic improvements are urgently needed.
                        </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-8">
                <div class="relative p-4">
                    <div class="relative">
                        <!-- Canvas Chart -->
                        <canvas id="efeChart" height="300"></canvas>
                    </div>
                </div>
        ...
        <div class="flex justify-between mt-8 no-print">
            ...
            <a href="?step=matrix-ife&key=<?= $_GET['key'] ?>" class="btn btn-soft-secondary" style="padding-left: 24px;padding-right: 24px;">
                Previous
            </a>
            <a href="?step=matrix-ie&key=<?= $_GET['key'] ?>" class="btn gradient-primary text-white" style="padding-left: 24px;padding-right: 24px;">
                Next
            </a>
        </div>
...
<script>
  // ====== LOAD SWOT DATA FOR EFE MATRIX ======
  let efeData = { 
    opportunities: [
      { id: 1, description: 'Growing market demand for sustainable products', weight: 0.15, rating: 4 },
      { id: 2, description: 'Expansion into emerging markets', weight: 0.12, rating: 3 },
      { id: 3, description: 'Strategic partnerships with suppliers', weight: 0.10, rating: 3 }
    ],
    threats: [
      { id: 4, description: 'Economic recession affecting consumer spending', weight: 0.18, rating: 2 },
      { id: 5, description: 'Increased competition from new entrants', weight: 0.15, rating: 2 },
      { id: 6, description: 'Regulatory changes in industry standards', weight: 0.12, rating: 1 }
    ]
  };
  ...
  async function loadEFEData() {
    try {
      ...
      if (result.success) {
        ...
        console.error('Failed to load EFE data:', result.message);
      }
    } catch (error) {
  ...
  function populateEFETable() {
    const tbody = document.getElementById('efeTableBody');
    if (!tbody) {
        console.error('Element with ID "efeTableBody" not found');
        return;
    }
    ...
    // Opportunities rows
    if (efeData.opportunities && efeData.opportunities.length > 0) {
        efeData.opportunities.forEach((item, index) => {
            html += createEFERow(index + 1, item, 'opportunity');
        });
    } else {
        html += createEmptyRow('No opportunities data available');
    }
    ...
    // Threats rows
    if (efeData.threats && efeData.threats.length > 0) {
        efeData.threats.forEach((item, index) => {
            html += createEFERow(index + 1, item, 'threat');
        });
    } else {
        html += createEmptyRow('No threats data available');
    }
  ...
  function createEFERow(number, item, type) {
      ...
      <tr class="bg-white dark:bg-gray-800" data-item-id="${item.id || ''}">
        ...
        <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200 align-top">
          <select name="rating" class="rating-select w-[100px] px-4 py-2 border border-gray-300 text-xs rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 bg-gray-100 dark:bg-gray-700 focus:bg-white dark:focus:bg-gray-900"
                  onchange="updateCalculations()">
            <option value="1" ${parseInt(item.rating || defaultRating) === 1 ? 'selected' : ''}>1 - Poor Response</option>
            <option value="2" ${parseInt(item.rating || defaultRating) === 2 ? 'selected' : ''}>2 - Below Average Response</option>
            <option value="3" ${parseInt(item.rating || defaultRating) === 3 ? 'selected' : ''}>3 - Above Average Response</option>
            <option value="4" ${parseInt(item.rating || defaultRating) === 4 ? 'selected' : ''}>4 - Superior Response</option>
          </select>
        </td>
  ...
  // ====== CONFIGURASI CHART ======
  const ctx = document.getElementById('efeChart');
  ...
  const chart = new Chart(ctx, {
    ...
    data: {
      datasets: [{
        label: 'EFE Score',
        data: [{ x: 0.5, y: efeScore }], // x=0.5 untuk titik di tengah sumbu-X
        showLine: false,
        pointRadius: 5,
        pointHoverRadius: 6,
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => `EFE: ${ctx.raw.y.toFixed(2)} / 4.0`
          }
        },
      ...
      scales: {
        x: {
          type: 'linear',
          min: 0, max: 1, // kita pakai 0..1, lalu label "EFE" di tengah
          grid: {
            display: false
          },
          ticks: {
            callback: (val) => (val === 0.5 ? 'EFE' : ''),
            maxTicksLimit: 3
          },
          border: {
  ...
  function showOverlay(show = true) {
    const overlay = document.getElementById('efeOverlay');
    const tooltip = document.getElementById('efeTooltip');
    const txt = document.getElementById('efeScoreText');
    if (show) {
      txt.textContent = (efeScore ?? 0).toFixed(2);
  ...
  // panggil setEFEScore(2.73)
  function setEFEScore(v) {
    efeScore = Math.max(0, Math.min(4, Number(v) || 0));
    chart.data.datasets[0].data = [{ x: 0.5, y: efeScore }];
    chart.update();
    
    // Update score display
    const scoreBadge = document.querySelector('.w-12.h-12 span');
    if (scoreBadge) {
      scoreBadge.textContent = efeScore.toFixed(2);
    }
    
    // Update interpretation
    updateScoreInterpretation(efeScore);
  }
  ...
  function updateScoreInterpretation(score) {
    ...
    if (score >= 3.5) {
      interpretation = 'Strong external position. Excellent strategic capabilities.';
      ...
      interpretation = 'Above average external position. Good strategic foundation.';
      colorClass = 'text-green-500';
      iconSvg = `<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ${colorClass}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>
      </svg>`;
    } else if (score >= 2.5) {
      interpretation = 'Average external position. Balanced strengths and weaknesses.';
      ...
      interpretation = 'Below average external position. Improvement needed.';
      colorClass = 'text-yellow-500';
      iconSvg = `<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ${colorClass}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4m0 4h.01M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/>
      </svg>`;
    } else {
      interpretation = 'Weak external position. Strategic improvements are urgently needed.';
    ...
    // Update icon and text color in external Factor Score section
  ...
  // Save EFE Data function
  // Bagian JavaScript (near existing <script> where saveEFEData is defined)
  let isEFEDirty = false;
  
  async function saveEFEData() {
      const saveButton = document.getElementById('saveEFEData');
      ...
      try {
        ...
        const tableRows = document.querySelectorAll('#efeTableBody tr[data-item-id]');
        ...
        tableRows.forEach(row => {
          ...
          if (itemId && weightInput && ratingSelect) {
            ...
            const opportunitiesHeader = document.querySelector('td[colspan="5"].bg-soft-primary');
            const threatsHeader = document.querySelector('td[colspan="5"].bg-soft-warning');
            
            if (opportunitiesHeader && threatsHeader) {
              const opportunitiesIndex = Array.from(opportunitiesHeader.parentNode.parentNode.children).indexOf(opportunitiesHeader.parentNode);
              const threatsIndex = Array.from(threatsHeader.parentNode.parentNode.children).indexOf(threatsHeader.parentNode);
              const currentIndex = Array.from(row.parentNode.children).indexOf(row);
              
              if (currentIndex > opportunitiesIndex && currentIndex < threatsIndex) {
                opportunitiesData.push(itemData);
              } else if (currentIndex > threatsIndex) {
                threatsData.push(itemData);
              }
            }
          }
        });
        ...
        const response = await fetch('<?= base_url('api/project/matrix_efe_save') ?>', {
          method: 'POST',
          headers: {
        ...
        if (result.success) {
          Swal.fire({
            icon: 'success',
            title: 'Berhasil!',
            text: 'Data EFE berhasil disimpan!',
        ...
        console.error('Error saving EFE data:', error);
        Swal.fire({
          icon: 'error',
          title: 'Oops...',
  ...
  function exportToExcel() {
    // Get the EFE table body
    const tableBody = document.getElementById('efeTableBody');
    if (!tableBody) {
    ...
    // Add header row
    data.push(['No', 'External Factor', 'Weight', 'Rating', 'Weight Score']);
    ...
    if (data.length <= 1) {
      alert('No data to export! Please add some external factors first.');
      return;
    }
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'EFE Matrix');
    ...
    const filename = `EFE_Matrix_${dateStr}.xlsx`;
  ...
  // Contoh pemakaian:
  // setEFEScore(0); showOverlay(true);
</script>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const inputs = document.querySelectorAll('#efeTableBody .weight-input, #efeTableBody .rating-select');
    inputs.forEach(el => {
      el.addEventListener('input', () => { isEFEDirty = true; });
      el.addEventListener('change', () => { isEFEDirty = true; });
    });

    // Target ONLY the bottom Next button inside the no-print footer
    const nextLink = document.querySelector('.no-print a.btn.gradient-primary[href*="step=matrix-ie"]');
    if (nextLink) {
